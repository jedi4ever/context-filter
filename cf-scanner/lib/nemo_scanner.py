        ...
        if content_length > 10 * 1024 * 1024:  # 10MB max
            ...
            if len(content) == content_length:
                ...
                try:
                    ...
                    print(f"[nemo] Scanning: {filename} ({content_length} bytes)", file=sys.stderr)
                    response = scan_content(text)
                    print(f"[nemo] Result: {filename} injection={response.get('is_injection')}, risk={response.get('risk_score', 0):.2f}", file=sys.stderr)
                except Exception as e:
